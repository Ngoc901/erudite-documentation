...
> The **Strategy Pattern** is highlighted in purple in the [Class Diagram](#class-diagram-manual) below. Full implementation details and rationale: [Week 17 blog entry](https://eruditedev.wordpress.com/2026/04/27/erudite-week-17/).

| Pattern                        | Class(es)                                                                                        | File                                   | Purpose                                                                                                                                               |
| ------------------------------ | ------------------------------------------------------------------------------------------------ | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **⬡ Strategy** _(highlighted)_ | `ScoringStrategy`, `ExactMatchScoringStrategy`, `PartialCreditScoringStrategy`, `ScoringContext` | `core/patterns/scoring_strategy.py`    | Interchangeable scoring algorithms - exact match for quiz/text, weighted partial credit for code challenges. Selected at runtime by `challenge_type`. |
| **Factory**                    | `GraderFactory`                                                                                  | `core/patterns/grader_factory.py`      | Resolves the correct grader key (`quiz_mcq`, `quiz_text`, `text`, `code`) from a challenge type, keeping dispatch logic in one place                  |
| **Observer**                   | `EnrollmentEventBus`, `EnrollmentObserver`, `EnrollmentAuditLogObserver`                         | `core/patterns/enrollment_observer.py` | Decouples enrollment side-effects (audit logging, future notifications) from the enrollment view                                                      |
| **Builder**                    | `CertificateBuilder`                                                                             | `core/patterns/certificate_builder.py` | Step-by-step construction of certificate PDF parameters before calling the generator                                                                  |
...
> The **Strategy Pattern** classes (`ScoringStrategy`, `ExactMatchScoringStrategy`, `PartialCreditScoringStrategy`, `ScoringContext`) are highlighted in **purple** in the diagram above. All other colours represent ORM model groups (see legend in diagram).
...
The Strategy Pattern was introduced to eliminate a growing `if/elif` chain in the challenge submission view where scoring logic differed by challenge type. By extracting each algorithm into its own class that implements `ScoringStrategy`, adding a new challenge type requires only a new class - the existing code is never touched (Open/Closed Principle).

![Strategy Pattern](../Images/SAD-Images/StrategyPattern.drawio.svg)

**Source:** [`core/patterns/scoring_strategy.py`](https://github.com/coffee3333/erudite-django-web-app/blob/main/core/patterns/scoring_strategy.py)

| Class                          | Role                                                                                             |
| ------------------------------ | ------------------------------------------------------------------------------------------------ |
| `ScoringStrategy`              | ABC - defines `compute_score(challenge, is_correct, hint_used): int`                             |
| `ExactMatchScoringStrategy`    | Full points if correct; 50% if hint was used. For `quiz_mcq`, `quiz_text`, `text`.               |
| `PartialCreditScoringStrategy` | Weighted by test-case results (`_earned / _total`). For `code` challenges.                       |
| `ScoringContext`               | Holds the active strategy; `compute()` delegates to it. Selected at runtime by `challenge_type`. |
